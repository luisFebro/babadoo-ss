...
export default function KeyAccessDashboard() {
    ...
    return (
        <div>
            {isUserAuthenticated ?
                (name === "admin") ?
                    <span className = "badge badge-primary shadow-elevation" style={{position: 'absolute', left: "10px", top: "-13px", zIndex: 1500}}>
                        <i style={{fontSize: '1.3rem'}} className="fas fa-key" onClick={null}>
                            <span className="text-border-black pl-3" style={{whiteSpace: 'nowrap'}}>Modo Admin</span>
                        </i>
                    </span> : null
                    :
                null
