...
export default function MultiIconButton({ top, right, left, bottom, onClick, size, buttonIcon, backColor }) {
  ...
  return (
      <Fab
        onClick={onClick}
        size={size || "small"}
        style={{position: 'absolute', top: `${top || 0}px`, right: `${right || 0}px`, left: `${left || 0}px`, bottom: `${bottom || 0}px`, zIndex: 1500, outline: 'none', color: 'var(--mainWhite)', backgroundColor: `${backColor}`}}
        aria-label="edit"
